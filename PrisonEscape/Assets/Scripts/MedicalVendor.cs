...
public class MedicalVendor : MonoBehaviour
{
    ...
    private void getInput(bool ctrlPress)
    {
        ...
        if (menuOpen && Input.GetButtonDown("AButton") || Input.GetKeyDown(KeyCode.Return))
        {
            ...
            else if(selected == 1)
            {
                ...
                {
                    GameManager.GetPlayer().getInventory().AddItem(new FirstAidKit());
                    GameManager.GetPlayer().setBalance(GameManager.GetPlayer().getBalance() - 8);
                    Debug.Log("Bought first aid kit");
                }
            }
            else if(selected == 2)
            {
                //buy baton
                if (GameManager.GetPlayer().getBalance() >= 20)
                {
                    GameManager.GetPlayer().getInventory().AddItem(new AdrenalineShot());
                    GameManager.GetPlayer().setBalance(GameManager.GetPlayer().getBalance() - 20);
                    Debug.Log("Bought adrenaline shot");
